...
def list_class():
    """ API to fetch the list of classes """
    return 'Nah !!!'
...
def scope_based_authorization():
    ...
    if access_token:
        ...
        try:
            keycloak_client.decode_jwt(access_token)
        except Exception:
            del session['access_token']
            return redirect(request.path)

        # retrieve rpt
        rpt_token = session.get('rpt_token') or keycloak_client.retrieve_rpt(access_token)['access_token']

        # decode rpt token
        try:
            token_info = keycloak_client.decode_jwt(rpt_token)
        except Exception:
            del session['rpt_token']
            return redirect(request.path)
    ...
    for path, rule in rules.items():
        if path == request.path:
            if rule[request.method.lower()] not in user_scopes:
                return Response(status=403)


# register middleware
api.before_request(scope_based_authorization)
