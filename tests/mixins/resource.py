...
def test_pat(mock_post, keycloak_client):
    ...
    headers = {
        "Authorization": "Basic Zmxhc2stYXBwOjFmNDlmMDU3LWJiZjktNDM4OS1hOTBmLTNjNTk3MmY1NTY0YQ=="
    }
    keycloak_client.pat
    mock_post.assert_called_once_with(
        keycloak_client.config.token_endpoint, data=payload, headers=headers
    )
...
@patch(
    "keycloak.mixins.resource.ResourceMixin.pat_auth_header", new_callable=PropertyMock
)
def test_list_resource(mock_headers, mock_get, keycloak_client):
    ...
    mock_get.assert_called_once_with(
        keycloak_client.config.resource_registration_endpoint, headers=headers
    )
...
def test_create_resource(mock_headers, mock_post, keycloak_client):
    ...
    mock_post.assert_called_once_with(
        keycloak_client.config.resource_registration_endpoint,
        json=resource,
        headers=headers,
    )
    mock_post.return_value.json.assert_called_once()
